...
class context_inpainting_dataloader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        canny = cv2.Canny(img, 120, 160)
        edge = -np.ones((image.shape[0], image.shape[1], 3), np.uint8)
        edge[:,:,0]=canny[:,:]
        edge[:,:,1]=canny[:,:]
        edge[:,:,2]=canny[:,:]
        ...
        input_, mask, image, edge = self.transform(mask, image, edge)

        return input_, mask, image

    def transform(self, mask, image, canny):
        ...
        canny = canny.transpose(2, 0, 1)
        canny = torch.from_numpy(canny.copy())

        return input_, mask, image, canny
