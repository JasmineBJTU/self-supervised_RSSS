...
def conv3x3(in_planes, out_planes, stride=1, atrous=1):
    ...
    return nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=1, dilation=atrous, bias=False)
...
class FCNify_v2(nn.Module):
    def __init__(self, original_model, n_class=21, layers_to_remove=['avgpool', 'fc']):
        super(FCNify_v2, self).__init__()
        torch.cuda.manual_seed(7)
        torch.manual_seed(7)
        for layers_ in layers_to_remove:
            #del(original_model.modules[layers_])
            del(original_model._modules[layers_])
        ...
        self.d6 = nn.Conv2d(2048, n_class, 1)

        self.upsample2 = bilinear(scale_factor = 2)
        self.upsample4 = bilinear(scale_factor = 4)
        self.upsample8 = bilinear(scale_factor = 8)
        self.upsample16 = bilinear(scale_factor = 16)
        self.upsample32 = bilinear(scale_factor=32)
    ...
    def forward(self, x):
        ...
        layer3 = self.features.layer3(layer2) #downsample 2^4
        layer4 = self.features.layer4(layer3) #downsample 2^5
        #layer5 = self.features.layer5(layer4)
        score_d1 = self.upsample32(self.d6(layer4))
        #layer6 = self.features.layer6(layer5)
        #layer7 = self.features.layer7(layer6)
        #layer8 = self.features.layer8(layer7)
        """
        d1 = self.features.relu(self.features.bn_d1(self.features.deconv1(layer4)))
        score_d1 = self.upsample8(self.d1(d1))
        d1 = self.features.relu(self.features.bn_d2(self.features.deconv2(d1)))
        score_d2 = self.upsample4(self.d2(d1))
        d1 = self.features.relu(self.features.bn_d3(self.features.deconv3(d1)))
        score_d3 = self.upsample2(self.d3(d1))
        ...
        score = crop(score_d1, rows, cols)
        return score


class FCNify_v3(nn.Module):
    def __init__(self, original_model, n_class=21, layers_to_remove=['classifier', 'tanh']):
        super(FCNify_v3, self).__init__()
        torch.cuda.manual_seed(7)
        torch.manual_seed(7)
        #for layers_ in layers_to_remove:
            #del(original_model._modules[layers_])
    ...
    def forward(self, x):
        ...
        #d1 = self.features.relu(self.features.deconv1(layer4))
        score_d1 = self.upsample32(self.d6(layer4))
        ...
        """
        score = crop(score_d1, rows, cols)
        # score = crop(score_d1, rows, cols) + crop(score_d2, rows, cols) + crop(score_d3, rows, cols)
        return score


class ResNet_Atrous_EncoderDecoder(nn.Module):

    def __init__(self, block, layers, atrous=None, os=16):
        super(ResNet_Atrous_EncoderDecoder, self).__init__()
        stride_list = None
        if os == 8:
            stride_list = [2, 1, 1]
        elif os == 16:
...
def resnet50_atrous_encoderdecoder(**kwargs):
    """
    Construct a ResNet-50 atrous-encoder+decoder model.
    :param kwargs:
    :return:
    """
    model = ResNet_Atrous_EncoderDecoder(atrous_Bottleneck, [3, 4, 6, 3], atrous=[1, 2, 1], os=16, **kwargs)
    return model

def resnet18_encoderdecoder(**kwargs):
    """Constructs a ResNet-50 encoder + decoder model.
    """
    model = ResNet_EncoderDecoder(BasicBlock, [2, 2, 2, 2], atrous=[2,2,1] ,**kwargs)
